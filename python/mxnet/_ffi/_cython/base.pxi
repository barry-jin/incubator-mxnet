...
from libc.stdint cimport int32_t, int64_t, uint64_t, uint8_t, uint16_t, uint32_t, uintptr_t
...
cdef inline int _convert_params(object arg,
    ...
    # cdef vector[const char*] params
    # params.resize(cdict.size())
    # for i in range(cdict.size()):
    #     params[i] = cdict[i].c_str()

    # value[0].v_handle = <void*>(CBeginPtr(params))
    # value[0].v_handle = <void *>(<unsigned long long>&params[0])
    value[0].v_handle = &cdict[0]

    # cdef unsigned long long v_ptr = &params[0]
    # value[0].v_handle = <void*>(v_ptr)

    # value[0].v_strlist = &params[0]
